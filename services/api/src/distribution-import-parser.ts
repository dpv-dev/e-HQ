...
function sourcePeriodForRow(
  row: Readonly<Record<string, string>>,
  source: DistributionImportSource
): { readonly start: string; readonly end: string; readonly reportDate: string | null } | null {
  ...
  for (const candidateKey of candidateKeys) {
    ...
    const sourceValue = Object.entries(row).find(([key, value]) =>
      normalizeColumnKey(key) === normalizedCandidate && value.trim().length > 0
    )?.[1] ?? null;
