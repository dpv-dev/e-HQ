...
| Suspense | `App.svelte` | `GET /erh/v1/suspense/workbench`, `POST /suspense/:id/resolve` | Postgres `suspense_items` + append-only resolution overrides + domain reason playbook | OK (release-ready) | Implemented 2026-07-20: the workbench reads the complete workspace-scoped Postgres queue rather than the bounded startup snapshot. It supports server-side search, legacy batch reference/file filtering, reason/status/date filters, multi-currency exposure KPIs, reason playbook, opaque keyset pagination, and exact-decimal CSV. Mapping, retry, and manual-resolution actions are workspace-scoped, locked, idempotent, audited, and append an immutable resolution record; retries for split blockers require an effective exact 100% split. Deployment and exact production-route QA remain pending. |
