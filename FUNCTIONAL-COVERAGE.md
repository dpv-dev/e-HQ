...
| --- | --- | --- | --- | --- | --- |
| Dashboard | `apps/hq/src/app/canonical/office/App.svelte` | `GET /eof/v1/dashboard`, `GET /eof/v1/screen/office` | `packages/domain-office/src/analytics.ts`, `pl.ts` | OK | Verified 2026-07-21: period/range is carried to live reads; presets, smart windows, and always-visible From/To inputs coexist. Bank quality, Recent imports, and Recent validated transactions remain removed by product decision. |
...
| Menu page | UI owner | API surface | Domain/data owner | Status | Next work |
| --- | --- | --- | --- | --- | --- |
| Dashboard | `apps/hq/src/app/canonical/distribution/App.svelte` | `GET /erh/v1/dashboard` | `domain-distribution` reads | OK | Financial KPIs respect the selected import-date range; readiness remains explicitly workspace-wide; All imported data uses live import-batch bounds; split coverage is separate from contract-backed coverage. |
| Imports | `App.svelte` | `/erh/v1/imports/batches`, preview/confirm/reverse | Distribution imports | OK | Kontor/RouteNote columns normalize into exact-decimal `normalized_earnings`; source rows and classified issues remain immutable; duplicate file fingerprints are rejected; deterministic mapping rules may match one target; batch status, audit, idempotency, and reversal are wired. |
...
| Contracts / advances | `App.svelte` | `/contracts/workbench`, `/contracts/track-rule-overrides`, expenses, `/payees` | Distribution contracts + recoupments | OK | Implemented and live-verified 2026-07-20: the track-centric workbench matches the reference workflow with active/effective/expired/draft, rule-quality, unallocated-row, and open-recoupment KPIs; server-side search/status/workflow filters; opaque keyset pagination; per-track and bulk split editing; and expense/advance editing. Imported rules remain immutable: corrections are append-only, administrator-only snapshots with exact 100% validation, idempotency, workspace/payee validation, Postgres persistence, startup projection overlay, and an audit event. Production QA confirmed `Spica` search, the Needs attention queue, cursor loading from 100 to 165 rows, an existing exact 40/60 split, all six expense categories, and zero browser diagnostic errors. Expenses can target artists, staff, suppliers, freelancers, or any other payee, and carry recoverable/date/description fields. |
...
| Allocations | `App.svelte` | `/allocations/workbench`, `/allocations/runs`, preview/post/unpost, `/allocations/retry-missing-contracts` | `domain-distribution/src/allocation.ts` + finance FX/allocation | OK | Implemented and live-verified 2026-07-20: the page now matches the reference workbench with live readiness and allocation-health KPIs, suspense reasons, recent runs, a cursor-paginated batch queue, and the cursor-paginated Unallocated Royalty Bank grouped by track and currency. Preview and execution can target one batch or the safe pending wave. Missing-contract rows can only be retried after an active exact 100% split exists; the reset is workspace-scoped, locked, idempotent, persisted, and audited. Calculation runs and suspense rows now persist `workspace_id`, earning statuses advance atomically, imported batches use their operational creation date when the legacy import date is absent, and all amounts remain textual/multi-currency. Exact FIFO recoupment, including dated cross-currency cost recovery, remains domain-owned; missing rates route earnings to suspense. Preview/post/unpost remain locked, idempotent, reversible, and audited. Production QA confirmed 28,864 ready rows, 98,102 open suspense rows, 98,090 missing-contract rows, the grouped `NIGHT & DAY` EUR royalty row, server-side search restoration, and zero browser warning/error logs. |
| Suspense | `App.svelte` | `GET /erh/v1/suspense/workbench`, `POST /suspense/:id/resolve` | Postgres `suspense_items` + append-only resolution overrides + domain reason playbook | OK | Live-verified 2026-07-20: the workbench reads the complete workspace-scoped Postgres queue rather than the bounded startup snapshot. It supports server-side search, legacy batch reference/file filtering, reason/status/date filters, multi-currency exposure KPIs, reason playbook, opaque keyset pagination, and exact-decimal CSV. Mapping, retry, and manual-resolution actions are workspace-scoped, locked, idempotent, audited, and append an immutable resolution record; retries for split blockers require an effective exact 100% split. |
| Statements | `App.svelte` | `/statements`, generate/print/void | Distribution statements | OK (live-verified 2026-07-20) | Lifecycle counts show draft, locked-due (`posted`), paid, and voided declarations. Each active row exposes immutable detail lines, exact-decimal CSV, A4 print, and audited removal with a mandatory reason. Removal preserves source calculations, hides the declaration from the active list, and is blocked while an active payment remains linked; it remains auditable as `void`. Production route QA confirmed the active-list action is labelled `Remove` and the retired `Void` action is absent. |
...
| Audit log | `App.svelte` | `GET /erh/v1/audit-log` | Postgres `audit_logs` filtered to Distribution actions | OK | Persisted events expose actor, action, entity, idempotency key, and context; date/actor/entity filters are server-side and the visible result exports to exact CSV. |
| Settings | `App.svelte` | `GET /erh/v1/settings`, `GET/POST /erh/v1/fx-rates`, `GET /erh/v1/status` | Distribution workspace config + finance FX rates | OK | Runtime write-gate state is visible and refreshable; currencies, counts, and FX history are live; dated FX rates save through a validated, idempotent, audited write. |
