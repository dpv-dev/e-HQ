...
| Statements | `App.svelte` | `/statements`, generate/print/void | Distribution statements | OK (release-ready) | Lifecycle counts show draft, locked-due (`posted`), paid, and voided declarations. Each active row exposes immutable detail lines, exact-decimal CSV, A4 print, and audited removal with a mandatory reason. Removal preserves source calculations, hides the declaration from the active list, and is blocked while an active payment remains linked; it remains auditable as `void`. Production deploy/route QA for this removal behaviour remains pending. |
