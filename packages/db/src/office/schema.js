...
import { bigint, boolean, char, date, index, integer, jsonb, pgEnum, pgTable, text, timestamp, uniqueIndex, uuid, varchar, check } from "drizzle-orm/pg-core";
...
export const categories = pgTable("categories", {
    ...
    divisionId: uuid("division_id")
        .references(() => divisions.id, { onDelete: "restrict", onUpdate: "cascade" }),
    isActive: boolean("is_active").notNull().default(true),
    createdAt: createdAtColumn()
}, (table) => [
...
export const financialAllocations = pgTable("financial_allocations", {
    id: uuid("id").defaultRandom().primaryKey(),
    ...
    check("financial_allocations_percentage_bp_check", sql `${table.percentageBp} >= 0 and ${table.percentageBp} <= 10000`)
]);
export const sharedCostRules = pgTable("shared_cost_rules", {
    id: uuid("id").defaultRandom().primaryKey(),
...
export const officeBankAccounts = pgTable("office_bank_accounts", {
    id: uuid("id").defaultRandom().primaryKey(),
...
export const officeBankImportBatches = pgTable("office_bank_import_batches", {
    id: uuid("id").defaultRandom().primaryKey(),
    workspaceId: text("workspace_id").notNull(),
...
export const officeBankStatementLines = pgTable("office_bank_statement_lines", {
    id: uuid("id").defaultRandom().primaryKey(),
    importBatchId: uuid("import_batch_id")
        .notNull()
        .references(() => officeBankImportBatches.id, { onDelete: "cascade", onUpdate: "cascade" }),
    ...
    rawData: jsonb("raw_data").$type().notNull().default(sql `'{}'::jsonb`),
    createdAt: createdAtColumn()
}, (table) => [
...
export const officeBankReconciliationMatches = pgTable("office_bank_reconciliation_matches", {
    id: uuid("id").defaultRandom().primaryKey(),
    bankStatementLineId: uuid("bank_statement_line_id")
        .notNull()
        .references(() => officeBankStatementLines.id, { onDelete: "cascade", onUpdate: "cascade" }),
    transactionId: uuid("transaction_id")
        .notNull()
        .references(() => transactions.id, { onDelete: "cascade", onUpdate: "cascade" }),
...
export const officeCashflowProjectionRows = pgTable("office_cashflow_projection_rows", {
    id: uuid("id").defaultRandom().primaryKey(),
