<script lang="ts">
  ...
  interface Props {
    ...
    // Optional per-row action buttons (e.g. "Edit", "Cancel"): rendered in a trailing
...
<section class={`ehq-table-shell ehq-edge-surface ${props.state}`} aria-label={props.title}>
  <header>
    <div class="title-stack">
      <p class="eyebrow ehq-type-label-mono">List</p>
  ...
  {#if props.state === "loading"}
    <div class="state-card">
      <Loader label="Loading list" detail="Preparing rows." size="medium" />
    </div>
  ...
  {:else if props.state === "error"}
    <div class="state-card error-copy">
      <strong>List unavailable</strong>
      <span>The request failed. Your current context has been preserved.</span>
    </div>
    ...
    {#if showFooter}
      <footer class="table-pagination" aria-label="List pagination">
        ...
        <div class="pagination-actions">
          {#if showNumberedPagination}
            <div class="pagination-numbers" aria-label="List pages">
