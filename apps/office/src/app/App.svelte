<script lang="ts">
  ...
  import { createOfficePreviewClient, officePreviewWorkspaceId } from "./office-preview-api.js";
  ...
  const client = createOfficePreviewClient();
  ...
  let activePageId = $state<OfficePageId>("pnl");
  ...
  async function loadDashboard(): Promise<void> {
    ...
    try {
      const dashboard = await client.office.getDashboard({
        workspaceId: officePreviewWorkspaceId,
        period
      });
  ...
  function readPageIdFromPath(pathname: string): OfficePageId {
    if (pathname.endsWith("/office/clients")) {
      return "clients";
    }

    if (pathname.endsWith("/office/suppliers")) {
      return "suppliers";
    }

    if (pathname.endsWith("/office/projects")) {
      return "projects";
    }

    if (pathname.endsWith("/office/monitoring")) {
      return "monitoring";
    }

    if (pathname.endsWith("/office/transactions")) {
      return "transactions";
    }

    if (pathname.endsWith("/office/imports")) {
      return "imports";
    }

    if (pathname.endsWith("/office/reconciliation")) {
      return "reconciliation";
    }

    if (pathname.endsWith("/office/pending")) {
      return "pending";
    }

    if (pathname.endsWith("/office/cashflow")) {
      return "cashflow";
    }

    if (pathname.endsWith("/office/chart-of-accounts")) {
      return "coa";
    }
  ...
  function pagePath(pageId: OfficePageId): string {
    ...
    if (pageId === "imports") {
      return "/office/imports";
    }

    if (pageId === "reconciliation") {
      return "/office/reconciliation";
    }
    ...
    if (pageId === "cashflow") {
      return "/office/cashflow";
    }

    if (pageId === "coa") {
      return "/office/chart-of-accounts";
    }
...
<main class="office-shell">
  <aside class="sidebar" aria-label="Office navigation">
    <button class="brand" type="button" onclick={() => selectPage("pnl")}>
    ...
    <p class="system-status ehq-type-label-mono"><span aria-hidden="true"></span>eof/v1 · preview</p>
  </aside>
  ...
  <section class="main-panel">
    <header class="topbar">
      ...
      <button class="profile" type="button" aria-label="Office profile">
        <span class="ehq-type-label-mono">DV</span>
        <strong class="ehq-type-heading">David</strong>
        <small class="ehq-type-label-mono">administrator</small>
      </button>
    </header>
    ...
    <div class="content">
      ...
      {:else if activePageId === "projects"}
        <ProjectsView client={client.office} workspaceId={officePreviewWorkspaceId} {period} />
      {:else if activePageId === "monitoring"}
        <MonitoringView client={client.office} workspaceId={officePreviewWorkspaceId} {period} />
