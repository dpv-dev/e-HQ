<script lang="ts">
  ...
  const signedOutWorkspaceAccess = {
    workspaceId: "command-center",
    status: "locked",
    reason: "Sign in with an administrator preview profile."
  } satisfies ReturnType<typeof getWorkspaceAccess>;
  const permissionUsers: readonly PreviewPermissionUser[] = createPreviewPermissionUsers();
  ...
  const readinessItems: readonly ReadinessItem[] = [
    { label: "Office preview", detail: "eof/v1 client surfaces ready", tone: "success" },
    { label: "Distribution preview", detail: "erh/v1 client surfaces ready", tone: "success" },
  ...
  let session = $state<AuthSession | null>(createPreviewAuthSession("administrator"));

  const activePage = $derived(getNavItem(activePageId));
  const commandAccess = $derived(session === null ? signedOutWorkspaceAccess : getWorkspaceAccess(session, "command-center"));
  ...
  function readPageIdFromPath(pathname: string): CommandCenterPageId {
    if (pathname.endsWith("/command-center/users")) {
      return "users";
    }

    if (pathname.endsWith("/command-center/integrations")) {
      return "integrations";
    }

    if (pathname.endsWith("/command-center/settings")) {
      return "settings";
    }
  ...
  function pagePath(pageId: CommandCenterPageId): string {
    if (pageId === "users") {
      return "/command-center/users";
    }

    if (pageId === "integrations") {
      return "/command-center/integrations";
    }

    if (pageId === "settings") {
      return "/command-center/settings";
    }
  ...
  function createDashboardKpis(
    users: readonly PreviewPermissionUser[],
  ...
  function createUsersKpis(users: readonly PreviewPermissionUser[]): readonly CommandKpi[] {
    return [
      { label: "Members", value: String(users.length), detail: "preview directory", tone: "info", accent: true },
  ...
  function createSettingsKpis(rows: readonly SettingRow[]): readonly CommandKpi[] {
    return [
      ...
      { label: "Settings", value: String(rows.length), detail: "local preview", tone: "info", accent: false },
  ...
  function createPermissionRows(users: readonly PreviewPermissionUser[]): readonly TableRow[] {
    return users.map((user: PreviewPermissionUser): TableRow =>
  ...
  function permissionCell(user: PreviewPermissionUser, workspaceId: WorkspaceAppId): TableRow["cells"][number] {
  ...
  function countRole(users: readonly PreviewPermissionUser[], roleId: string): number {
    return users.filter((user: PreviewPermissionUser): boolean => user.roleId === roleId).length;
  }

  function countReviewUsers(users: readonly PreviewPermissionUser[]): number {
    return users.filter((user: PreviewPermissionUser): boolean => user.status === "review").length;
  }

  function countDeniedAccess(users: readonly PreviewPermissionUser[]): number {
    return users.reduce((total: number, user: PreviewPermissionUser): number => {
  ...
  function saveSettingsPreview(): void {
    commandNotice = "Settings preview checked locally. No deployment or remote write was triggered.";
  }

  function setPreviewSession(nextSession: AuthSession): void {
    session = nextSession;
    commandNotice =
      getWorkspaceAccess(nextSession, "command-center").status === "allowed"
        ? "Preview session switched. Command Center access is allowed."
        : "Preview session switched. Command Center is locked for this profile.";
  }

  function clearPreviewSession(): void {
    session = null;
    commandNotice = "Signed out. Command Center is locked until an administrator profile is selected.";
  }
...
<main class="command-shell">
  ...
  <section class="main-panel">
    <header class="topbar">
      ...
      <DevSessionMenu session={session} onSessionChange={setPreviewSession} onLogout={clearPreviewSession} />
    </header>
    ...
    <div class="content">
      ...
      {:else if activePageId === "users"}
        ...
        <section class="permission-workbench">
          <section class="form-panel ehq-edge-surface" aria-label="Invite preview">
            <h2>Access editor</h2>
            <p>Preview only. Persisted permission changes belong behind the API layer.</p>
      ...
      {:else}
        ...
        <section class="settings-grid">
          <section class="form-panel ehq-edge-surface" aria-label="Workspace settings">
            ...
            <p>Local preview controls only; saving here does not deploy or write remotely.</p>
            ...
            <button class="command-action" type="button" onclick={saveSettingsPreview}>Save preview</button>
          </section>
