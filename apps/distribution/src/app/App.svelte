<script lang="ts">
  ...
  import {
    createDistributionPreviewClient,
    distributionPreviewPeriod,
    distributionPreviewWorkspaceId
  } from "./distribution-preview-api.js";
  ...
  const client = createDistributionPreviewClient();
  const allValue = "all";
  const allocationLockKey = `distribution:allocations:${distributionPreviewPeriod}`;
  ...
  async function loadDashboard(): Promise<void> {
    ...
    try {
      dashboardState = createSuccessState<DistributionDashboardResponse>(
        await client.distribution.getDashboard({ workspaceId: distributionPreviewWorkspaceId, period: distributionPreviewPeriod })
      );
    } catch (error: unknown) {
    ...
    try {
      importBatchesState = createSuccessState<PageResult<DistributionImportBatch>>(
        await client.distribution.listImportBatches({
          workspaceId: distributionPreviewWorkspaceId,
  ...
  async function loadPayees(): Promise<void> {
    ...
    try {
      payeesState = createSuccessState<PageResult<PayeeSummary>>(
        await client.distribution.listPayees({ workspaceId: distributionPreviewWorkspaceId, status: null, cursor: null, limit: 50 })
      );
    } catch (error: unknown) {
  ...
  async function loadCatalog(): Promise<void> {
    ...
    try {
      const [releasePage, trackPage] = await Promise.all([
        client.distribution.listReleases({ workspaceId: distributionPreviewWorkspaceId, status: null, cursor: null, limit: 50 }),
        client.distribution.listTracks({ workspaceId: distributionPreviewWorkspaceId, releaseId: null, status: null, cursor: null, limit: 50 })
      ]);
    ...
    try {
      suspenseState = createSuccessState<PageResult<SuspenseItem>>(
        await client.distribution.listSuspense({
          workspaceId: distributionPreviewWorkspaceId,
          period: distributionPreviewPeriod,
    ...
    runReceipt = await client.distribution.generateStatements(
      {
        ...
        lockKey: `distribution:statements:${distributionPreviewPeriod}`
      },
      {
...
<main class="distribution-shell">
  <aside class="sidebar" aria-label="Navigation Distribution">
    ...
    <p class="system-status"><span aria-hidden="true"></span>erh/v1 · preview</p>
  </aside>
  ...
  <section class="main-panel">
    <header class="topbar">
      ...
      <button class="profile" type="button" aria-label="Distribution profile">
        <span>DV</span>
        <strong>David</strong>
    ...
    <div class="content">
      ...
      {:else if activePageId === "statements"}
        ...
        <section class="statement-pdf ehq-edge-surface" aria-label="A4 statement PDF preview">
          ...
          <p>Period {statementPreview?.period ?? distributionPreviewPeriod} · currency {statementPreview?.currency ?? "MUR"}</p>
